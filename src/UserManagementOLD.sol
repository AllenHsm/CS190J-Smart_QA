...
contract UserManagement {
    ...
    function calculate_credit(address userAddress) public returns (uint256) {
        ...
        emit UserCreditUpdate(userAddress, userAddrMap[userAddress].credit, sqr_sum);
        return sqrt(sqr_sum);
    }
    ...
    function getCredit(address userAddress) external view returns (uint256) {
        return userAddrMap[userAddress].credit;
    }

    function getMyCredit() external view returns (uint256) {
        require(
